...
3. 回答要清晰、分点，适合中文用户阅读。
4. 回答结尾必须列出你参考的资料来源，格式为：文件名 - 页码 - 片段编号。
...
def format_context_chunk(idx: int, item: dict) -> str:
...
def call_llm(question: str, retrieved_chunks: list[dict]) -> str:
    ...
    if not final_answer:
        return "模型没有返回有效内容，请检查模型名、API Key 权限或余额。"
